...
- Form fields: owner, repo, branch (default `main`), docs root (default `docs`), token, webhook secret optional.
- "Test connection" runs three checks and shows each result: auth works, repo and branch found, docs root lists at least one `.md` file.
...
- Test path uses `GET /repos/{owner}/{repo}/contents/{docsRoot}?ref={branch}`.
...
- Empty docs root: warn, allow save, show "no pages yet" in projects.
